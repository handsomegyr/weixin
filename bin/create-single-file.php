...
// -------------------------------------------------------------------------- //
// This script can be used to automatically glue all the .php files of Weixin
...
class WeixinFile
{
    ...
    public static function from($libraryPath, Array $exclude = array())
    {
        ...
        foreach (new RecursiveIteratorIterator($libIterator) as $classFile)
        {
            ...
            $phpNamespace = $WeixinFile->getNamespace($namespace);

            if ($phpNamespace === false) {
                $phpNamespace = new PhpNamespace($namespace);
                $WeixinFile->addNamespace($phpNamespace);
            }
...
LICENSE;
    ...
    public function getImplementedInterfaces($all = false)
    {
        ...
        return array_filter(
            $this->implements,
            function ($cn) { return strpos($cn, 'Weixin\\') === 0; }
        );
    }
